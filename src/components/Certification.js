import React from 'react'

const Certification = () => {
  return (


<div class=" wrap overflow-hidden p-10 h-full text-black bg-white">

<h2 class="text-center text-2xl font-bold m-5 ">Certifications</h2>
    <hr />
     <div class="container">
          <div
            class="flex flex-col md:grid grid-cols-5 mx-auto p-4 text-blue-900">
            <div class="flex flex-row-reverse md:contents">


              <div class="bg-white col-start-1 col-end-1  pr-8 rounded-xl my-4 ml-auto">
                <img class="leading-tight text-justify" src="https://img.icons8.com/ios/452/warranty.png"/>
                <h3>Founder</h3>
                <p class="font-bold  text-lg">Person name</p>
               </div>


               <div class="bg-white col-start-2  col-end-2  pr-8 rounded-xl my-4 ml-auto">
                <img class="leading-tight text-justify" src="https://img.icons8.com/ios/452/warranty.png"/>
                <h3>Director</h3>
                <p class="font-bold  text-lg">Person name</p>
               </div>


               <div class="bg-white col-start-3 col-end-3   pr-8 rounded-xl my-4 ml-auto">
                <img class="leading-tight text-justify" src="https://img.icons8.com/ios/452/warranty.png"/>
                <h3>Chief of Marketing</h3>
                <p class="font-bold text-lg">Person name</p>
               </div>


               <div class="bg-white col-start-4 col-end-4   pr-8 rounded-xl my-4 ml-auto">
                <img class="leading-tight text-justify" src="https://img.icons8.com/ios/452/warranty.png"/>
                <h3>Chief of Marketing</h3>
                <p class="font-bold text-lg">Person name</p>
               </div>


               <div class="bg-white col-start-5 col-end-45  pr-8 rounded-xl my-4 ml-auto">
                <img class="leading-tight text-justify" src="https://img.icons8.com/ios/452/warranty.png"/>
                <h3>Chief of Marketing</h3>
                <p class="font-bold text-lg">Person name</p>
               </div>


            </div>
          </div>
     </div>

     <hr />
</div>



  )
}

export default Certification;